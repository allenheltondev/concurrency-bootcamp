...
(function () {
  ...
  { id:"bug_rollrate", title:"Utilization meter", why:"'right now' needs a window, not a lifetime", lesson:17,
    ...
    bug:[13],
    explain:"Line 14 divides accumulated busy time by the time since the PROCESS STARTED — a lifetime average. Six quiet hours since the 3am deploy sit in the denominator forever, so ten minutes of full saturation reads as '45%', and every number the meter ever reports drifts toward the historical mean of the whole uptime. Utilization for operational decisions must be windowed: busy time within the last window divided by the window (rotate buckets, or decay the accumulator) — and attribute busy time as it accrues, not only at completion, or a long-running request hides until it finishes. A meter that includes last night's idle time cannot describe this minute's emergency." },
