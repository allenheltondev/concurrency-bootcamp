/* The shared auth session contract — a TypeScript port of js/account.js's
   auth core. Per ADR 0002 the seam shared with the vanilla course pages is
   exactly the `rsc:auth` localStorage document ({idToken, refreshToken,
   expiresAt}) plus the Cognito user pool API calls; a sign-in on either
   surface is a sign-in on both. Change the shape on both sides deliberately.
   No SDK, no Hosted UI — plain cognito-idp calls over TLS. */
...
/* ---------- friendly error translation (mirrors account.js ERROR_COPY) ---------- */
...
/** Parse the id token payload (base64url JWT) — same defensive read account.js uses. */
...
if (typeof window !== "undefined") {
  // a sign-in/out in another tab (course page or platform) updates this one
...
async function requireConfig(): Promise<AuthConfig> {
  ...
  if (!config) throw new AuthError("Accounts aren't enabled on this deployment.", "ConfigMissing");
  return config;
}
...
export async function signIn(email: string, password: string): Promise<SignInResult> {
  ...
  try {
    ...
    if (isAuthError(e) && e.code === "UserNotConfirmedException") {
      ...
      // on the confirm screen (mirrors account.js)
